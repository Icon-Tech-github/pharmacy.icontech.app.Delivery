...
bool ? isResend = false;
  ...
  logout() async {
    try {
      ...
      // استدعاء تسجيل الخروج من المستودع
      final data = await authRepo.logout(LocalStorage.getData(key: "token"));

      if (data == null) {
        emit(AuthFailure(error: "Logout failed: No data returned"));
      } else {
        // إزالة البيانات من LocalStorage
        await LocalStorage.removeData(key: "token");
        await LocalStorage.removeData(key: "userName");
